...
def writeSolverToJSON(solver, x_pin, y_pin, z_pin, x_assem, y_assem, z_assem, outputs_filepath: str):
    print('Making solver dictionary')
...
def readSolverFromJSON(outputFile: str, print_output=True):
    ...
    print(
        " Separating flux array into multigroup flux dictionary res.flux['flux0'] etc.")
    print(
        " Separating avg_flux array into multigroup flux dictionary res.avg_flux['avg_flux0'] etc.")
...
def plotFissRateCompareAssem_MOCMC(outputFileMC: str, binFileMOC: str, scarab_text_log: str, name='assembly'):
    ...
    # keff comparison
    keff_SCONE, keff_scarab, keff_diff = compareKeff_MOCMC(
        outputFileMC, scarab_text_log)
    ...
    plot.savefig('outputs/' + 'compare_MOC_MC_' +
...
def plotAllFromJSON(outputFileScarab: str, plotting=True):
    ...
    print('x_pin dimensions:', len(res.x_pin),
          'elements from', res.x_pin[0], 'to', res.x_pin[-1])
    print('y_pin dimensions:', len(res.y_pin),
          'elements from', res.y_pin[0], 'to', res.y_pin[-1])
    print('z_pin dimensions:', len(res.z_pin),
          'elements from', res.z_pin[0], 'to', res.z_pin[-1])
    print('x_assem dimensions:', len(res.x_assem),
          'elements from', res.x_assem[0], 'to', res.x_assem[-1])
    print('y_assem dimensions:', len(res.y_assem),
          'elements from', res.y_assem[0], 'to', res.y_assem[-1])
    print('z_assem dimensions:', len(res.z_assem),
          'elements from', res.z_assem[0], 'to', res.z_assem[-1])
    print('pin_xloc dimensions:', len(res.pin_xloc),
          'elements from', res.pin_xloc[0], 'to', res.pin_xloc[-1])
    print('pin_xloc dimensions:', len(res.pin_yloc),
          'elements from', res.pin_xloc[0], 'to', res.pin_yloc[-1])
    ...
    if plotting:
        ...
        plot.close()
    print('pin_power dimensions: ' + str(res.pin_power.shape) +
          ':: plot saved to ' + outputs_filepath + 'pin_power.svg')
    ...
    if plotting:
        ...
        plot.close()
    print('power_pinwise dimensions: ' + str(res.power_pinwise.shape) +
          ':: plot saved to ' + outputs_filepath + 'power_pinwise.svg')
    ...
    if plotting:
        ...
        plot.close()
    print('power_assembly_wise dimensions: ' + str(res.power_assembly_wise.shape) +
          ':: plot saved to ' + outputs_filepath + 'power_assembly_wise.svg')
    ...
    if plotting:
        ...
        plot.close()
    print('avg_power dimensions: ' + str(res.avg_power.shape) +
          ':: plot saved to ' + outputs_filepath + 'avg_power.svg')
...
def plotSpatialParamJSON(outputJSONScarab, paramName, normalise_by_mean='all', vmax=None, vmin=None, plotting=True, visualise_quarter=False, aspect_ratio=1, indices=None, mgID=None):
    ...
    """
    print('\n#########\nLogging: plotting {} from scarabee file {} '.format(
...
def plotSpatialParam_CompareNodalMC(outputFileMC, outputFileNodal, tallyName_MC, paramNameNodal, normalise_by_mean='all',  response_index_MC=0, vmax=None, vmin=None, plotting=True, visualise_quarter_MC=False, visualise_quarter_nodal=False, aspect_ratio=1, indicesNodal=None, mgID_nodal=None, annotate=False):
    ...
    value_MC, std_MC = plotSpatialTallyMC(outputFileMC, tallyName=tallyName_MC, normalise_by_mean=normalise_by_mean,
                                          response_index=response_index_MC, visualise_quarter=visualise_quarter_MC, plotting=False)
    value_nodal = plotSpatialParamJSON(outputFileNodal, paramNameNodal, normalise_by_mean=normalise_by_mean,
                                       plotting=False, visualise_quarter=visualise_quarter_nodal, indices=indicesNodal, mgID=mgID_nodal)
    ...
    if plotting:
        ...
        val_plot = ax1.imshow(rel_diff * 100, cmap='rainbow',
        ...
        if annotate == True:
            for (j, i), label in np.ndenumerate(rel_diff):
                ax1.text(i, j, '{:.2f}'.format(
                    label*100), ha='center', va='center', fontsize=4, color='white')
        ...
        fig.suptitle('(Nodal -MC) / MC: relative difference in {}. \nMax error={:.3%}, min error={:.3%}, \nRMSE={:.3%}.\n$k_{{MC}}={:.5f}$, $k_{{nodal}}={:.5f}$, diff={:.0f} pcm'.format(
            paramNameNodal, max_error, min_error, rmse, keff_MC, keff_nodal, 1e5*(keff_nodal-keff_MC)) + '\n' + quarter_label)
