...
def plotSpatialTallyRR(
    ...
    outputs = readOutputs(outputFileRR, print_output=True)

    if hasattr(outputs.tally, tallyName):
        pass
    else:
        logging.warning('!!! %s tally was not found, end of plotting !!!', tallyName)
        exit()

    activeTallyResults = getattr(outputs.tally, tallyName)
    ...
    if plotting:
        ...
        plot.savefig(newpath + '/' + modelName + tallyName +
                     str(int(response_index)) + label_energy_group + '_RR.svg')
        plot.close()

    return value, std


def plotSpatialTallyRR_v0(outputFileRR, tallyName, normalise_by_mean='all', vmax=None, vmin=None, vmax_unc=None, vmin_unc=None, plotting=True, visualise_quarter=False, aspect_ratio=1, orientation='horizontal', remove_edges_2D=False, newpath='outputs'):
...
def plot_MC_results(
    ...
    fissTally_pin = 'pinFiss'
    fissTally_assem = 'assemblyFiss'
    fluxIndex = 0
    fissIndex = 1
    ...
    # Flux
    # (pins)
    plotSpatialTallyMC(outputFileMC, fissTally_pin, normalise_by_mean=normalise_by_mean, response_index=fluxIndex,
    ...
    # (assemblies)
    plotSpatialTallyMC(outputFileMC, fissTally_assem, normalise_by_mean=normalise_by_mean, response_index=fluxIndex,
    ...
    # Fission rate
    # (pins)
    valFissPin, stdFissPin = plotSpatialTallyMC(outputFileMC, fissTally_pin, normalise_by_mean=normalise_by_mean, response_index=fissIndex,
    ...
    # (assemblies)
    valAssemPin, stdAssemPin = plotSpatialTallyMC(outputFileMC, fissTally_assem, normalise_by_mean=normalise_by_mean, response_index=fissIndex,
                       visualise_quarter=visualise_quarter, average_along_diag=average_along_diag, newpath=newpath, annotate=annotate, cmap_colour=cmap_colour, fontsize=fontsize, colour=colour, dp=dp, tol=tol)
    

    # Group-wise flux plots
    # (pins)
    fluxTally_pin = 'pinFlux'
    fluxTally_assembly = 'assemblyFlux'
    outputs = readOutputs(outputFileMC, print_output=False)
    outputs_active = outputs.active

    if hasattr(outputs_active, fluxTally_pin):
        activeTallyResults = getattr(outputs_active, fluxTally_pin)
        logging.info('Flux tallies detected')
 
        if hasattr(activeTallyResults, 'EnergyBounds'):
            energyBounds = activeTallyResults.EnergyBounds
            logging.info('Energy bounds %s detected for %s', energyBounds, fluxTally_pin)
            ngroups = len(list(energyBounds))
            logging.info('Finding the number of energy groups, %s', ngroups)

            for g in range(ngroups):
                plotSpatialTallyMC(outputFileMC, fluxTally_pin, normalise_by_mean=normalise_by_mean, response_index=0, group_index=g,
                                visualise_quarter=visualise_quarter, average_along_diag=average_along_diag, newpath=newpath, annotate=False, cmap_colour=cmap_colour, tol=tol)
                plotSpatialTallyMC(outputFileMC, fluxTally_assembly, normalise_by_mean=normalise_by_mean, response_index=0, group_index=g,
...
def plot_RR_results(
    ...
    # Flux
    # (pins)
    plotSpatialTallyRR(outputFileRR, fissTally_pin, normalise_by_mean=normalise_by_mean, response_index=fluxIndex,
                       visualise_quarter=visualise_quarter, average_along_diag=average_along_diag, newpath=newpath, annotate=False, cmap_colour=cmap_colour, modelName=modelName, tol=tol)
    # (assemblies)
    plotSpatialTallyRR(outputFileRR, fissTally_assem, normalise_by_mean=normalise_by_mean, response_index=fluxIndex,
                       visualise_quarter=visualise_quarter, average_along_diag=average_along_diag, newpath=newpath, annotate=annotate, cmap_colour=cmap_colour, fontsize=fontsize, colour=colour, dp=dp, tol=tol)

    # Fission rate
    # (pins)
    valFissPin, stdFissPin = plotSpatialTallyRR(outputFileRR, fissTally_pin, normalise_by_mean=normalise_by_mean, response_index=fissIndex,
                       visualise_quarter=visualise_quarter, average_along_diag=average_along_diag, newpath=newpath, annotate=False, cmap_colour=cmap_colour, tol=tol)
   
    # (assemblies)
    valAssemPin, stdAssemPin = plotSpatialTallyRR(outputFileRR, fissTally_assem, normalise_by_mean=normalise_by_mean, response_index=fissIndex,
                       visualise_quarter=visualise_quarter, average_along_diag=average_along_diag, newpath=newpath, annotate=annotate, cmap_colour=cmap_colour, fontsize=fontsize, colour=colour, dp=dp, tol=tol)
    ...
    if hasattr(outputs_active, fluxTally_pin):
        ...
        if hasattr(activeTallyResults, 'EnergyBounds'):
            ...
            for g in range(ngroups):
                plotSpatialTallyRR(outputFileRR, fluxTally_pin, normalise_by_mean=normalise_by_mean, response_index=0, group_index=g,
                                visualise_quarter=visualise_quarter, average_along_diag=average_along_diag, newpath=newpath, annotate=False, cmap_colour=cmap_colour, tol=tol)
                plotSpatialTallyRR(outputFileRR, fluxTally_assembly, normalise_by_mean=normalise_by_mean, response_index=0, group_index=g,
                                visualise_quarter=visualise_quarter, average_along_diag=average_along_diag, newpath=newpath, annotate=annotate, cmap_colour=cmap_colour, colour=colour, fontsize=fontsize)
        else:
            logging.info('No energy bounds detected')
    else:
