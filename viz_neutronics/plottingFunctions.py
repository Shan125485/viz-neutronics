...
def plot_RR_results(
    ...
    # Flux
    # (pins)
    plotSpatialTallyRR(outputFileMC, fissTally_pin, normalise_by_mean=normalise_by_mean, response_index=fluxIndex,
                       visualise_quarter=visualise_quarter, average_along_diag=average_along_diag, newpath=newpath, annotate=False, cmap_colour=cmap_colour, modelName=modelName, tol=tol)
    # (assemblies)
    plotSpatialTallyRR(outputFileMC, fissTally_assem, normalise_by_mean=normalise_by_mean, response_index=fluxIndex,
                       visualise_quarter=visualise_quarter, average_along_diag=average_along_diag, newpath=newpath, annotate=annotate, cmap_colour=cmap_colour, fontsize=fontsize, colour=colour, dp=dp, tol=tol)

    # Fission rate
    # (pins)
    valFissPin, stdFissPin = plotSpatialTallyRR(outputFileMC, fissTally_pin, normalise_by_mean=normalise_by_mean, response_index=fissIndex,
                       visualise_quarter=visualise_quarter, average_along_diag=average_along_diag, newpath=newpath, annotate=False, cmap_colour=cmap_colour, tol=tol)
   
    # (assemblies)
    valAssemPin, stdAssemPin = plotSpatialTallyRR(outputFileMC, fissTally_assem, normalise_by_mean=normalise_by_mean, response_index=fissIndex,
                       visualise_quarter=visualise_quarter, average_along_diag=average_along_diag, newpath=newpath, annotate=annotate, cmap_colour=cmap_colour, fontsize=fontsize, colour=colour, dp=dp, tol=tol)
    ...
    if hasattr(outputs_active, fluxTally_pin):
        ...
        if hasattr(activeTallyResults, 'EnergyBounds'):
            ...
            for g in range(ngroups):
                plotSpatialTallyRR(outputFileMC, fluxTally_pin, normalise_by_mean=normalise_by_mean, response_index=0, group_index=g,
                                visualise_quarter=visualise_quarter, average_along_diag=average_along_diag, newpath=newpath, annotate=False, cmap_colour=cmap_colour, tol=tol)
                plotSpatialTallyRR(outputFileMC, fluxTally_assembly, normalise_by_mean=normalise_by_mean, response_index=0, group_index=g,
                                visualise_quarter=visualise_quarter, average_along_diag=average_along_diag, newpath=newpath, annotate=annotate, cmap_colour=cmap_colour, colour=colour, fontsize=fontsize)
        else:
            logging.info('No energy bounds detected')
    else:
